...
public class Code03_ValidSortedArrayWays {
    ...
    public static long ways1(int[] nums, int k) {
        ...
        //对于 i 个格子 j 种数字的情况，方案数 =
        //最后一个格子填 j，前 i-1 个格子填 j 种数字的方案数（dp[i-1][j]）
        //最后一个格子不填 j（即最多填 j-1），i 个格子填 j-1 种数字的方案数（dp[i][j-1]）
        for (int i = 2; i <= n; i++) {
            for (int j = 2; j <= k; j++) {
    ...
    public static long c(int a, int b) {
        ...
        long x = 1;//分子
        long y = 1;//分母
        //组合数公式为 C (a,b) = a!/(b!・(a-b)!)
        //优化计算：直接计算 (a×(a-1)×...×(b+1)) / (1×2×...×(a-b))
