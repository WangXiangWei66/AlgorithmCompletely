...
public class Code07_IrregularSudoku {
    ...
    public static String solution(int[][] sudoku, int[][][] map) {
        boolean[][] row = new boolean[3][4];// 0索引不用
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (sudoku[i][j] != 0) {
                    ...
                    bucket[own[i][j]][sudoku[i][j]] = true;//所在区域已有num
                }
            }
        }
    ...
    //own：单元格所属区域数组
    public static int process(int[][] sudoku, int i, int j, boolean[][] row, boolean[][] col, boolean[][] bucket, int[][] own) {
        if (i == 3) {
            return 1;
        }
        int nexti = j != 2 ? i : i + 1;
        int nextj = j != 2 ? j + 1 : 0;
        //当前单元格已有数字，直接处理下一个单元格
        if (sudoku[i][j] != 0) {
            return process(sudoku, nexti, nextj, row, col, bucket, own);
        } else {
            int ans = 0;
            int bid = own[i][j];//当前单元格所属区域编号
