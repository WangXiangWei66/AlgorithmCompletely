...
public class Code02_ToAllSpace {
    ...
    public static int minDistance1(int[][] map) {
        ...
        int[][] nodes = new int[n][2];//存储每个多余棋子的初始位置
        int[][] spaces = new int[m][2];//存储每个空格子的目标位置
        n = 0;
        m = 0;
        for (int i = 0; i < map.length; i++) {
            for (int j = 0; j < map[0].length; j++) {
                ...
                for (int k = 2; k <= map[i][j]; k++) {
                    nodes[n][0] = i;//多余棋子的行坐标
                    nodes[n++][1] = j;//多余棋子的列坐标
                }
                //记录棋子数为0的格子位置
                if (map[i][j] == 0) {
                    spaces[m][0] = i;
        ...
        return process1(nodes, 0, spaces);
    }

    //nodes：多余棋子的位置数组
    //index：当前处理到的棋子索引
    //spaces：空格子位置数组，他的顺序从开始到结束都不会改变
    public static int process1(int[][] nodes, int index, int[][] spaces) {
        int ans = 0;
        if (index == nodes.length) {
            for (int i = 0; i < nodes.length; i++) {
                ans += distance(nodes[i], spaces[i]);
            }
        } else {
            ans = Integer.MAX_VALUE;
            for (int i = index; i < nodes.length; i++) {
                swap(nodes, index, i);
                ans = Math.min(ans, process1(nodes, index + 1, spaces));
                swap(nodes, index, i);
            }
        }
        return ans;
