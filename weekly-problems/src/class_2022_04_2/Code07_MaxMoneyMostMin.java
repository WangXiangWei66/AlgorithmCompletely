...
public class Code07_MaxMoneyMostMin {
    ...
    public static class SegmentTree {
        ...
        public SegmentTree(int maxSize) {
            ...
            max = new int[n << 2];
            update = new int[n << 2];
            Arrays.fill(update, -1);//初始化没有需要更新的操作
        }

        // 调用私有更新方法，更新[index, index]区间为c
        public void update(int index, int c) {
            update(index, index, c, 1, n, 1);
        }

        // 公开方法：查询[1, right]区间的最大值
    ...
    public static boolean year(long all, long[][] staff, long limit) {
        int n = staff.length;
        SegmentTree st = new SegmentTree(n);
        // 哈希表：存储相同value对应的员工索引
