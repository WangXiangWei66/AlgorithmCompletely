...
public class Code02_MinDistanceFromLeftUpToRightDown {
    ...
    public static int bestWalk2(int[][] map) {
        ...
        while (!heap.isEmpty()) {
            ...
            add(dis, row - 1, col, map[row][col], n, m, map, poped, heap);//上
            add(dis, row + 1, col, map[row][col], n, m, map, poped, heap);//下
            add(dis, row, col - 1, map[row][col], n, m, map, poped, heap);//左
            add(dis, row, col + 1, map[row][col], n, m, map, poped, heap);//右
        }
        return ans;
    }
