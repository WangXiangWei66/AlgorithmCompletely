...
public class Code01_MeetingCheck {
    ...
    public static boolean[] reserveMeetings(int m, int[][] meetings) {
        ...
        //收集每个会议的开始时间和结束时间-1（因为会议时间为左闭右开）
        for (int i = 0; i < n; i++) {
        ...
        int max = 0;//记录最大的压缩后坐标值
        for (int i = 0; i < n; i++) {
            reMeetings[i][0] = i;//用于最后的结果映射
            reMeetings[i][1] = meetings[i][0];//预定动作的发生时间
            //将大数映射为小得索引
            reMeetings[i][2] = ranks(ranks, meetings[i][1]);
            reMeetings[i][3] = ranks(ranks, meetings[i][2] - 1);
            max = Math.max(max, reMeetings[i][3]);
        }
        SegmentTree st = new SegmentTree(max);
        Arrays.sort(reMeetings, (a, b) -> (a[1] - b[1]));//确保早来早得
    ...
    public static int ranks(int[] sorted, int num) {
        int l = 0;
        ...
        return ans + 1;//线段树的处理索引从1开始
    }

    public static class SegmentTree {
        private int n;//线段树处理的数组大小
