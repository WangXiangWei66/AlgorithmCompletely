...
public class Code05_RedAndWhiteSquares {
    ...
    // ops：二维数组，每个元素是一个三元组(a,b,c)表示染色操作的坐标
    public static int[] blocks1(int n, int m, int h, int[][] ops) {
        int k = ops.length;//总操作次数
        // 三维数组表示立方体：
        // 0表示白色（初始状态）
        // -1表示被染成红色
        // 其他正整数表示已访问的白色连通块标记
    ...
    public static void infect(int[][][] cube, int a, int b, int c, int change) {
        if (a < 0 || a == cube.length || b < 0 || b == cube[0].length || c < 0 || c == cube[0][0].length || cube[a][b][c] == -1 || cube[a][b][c] == change) {
            return;
        }
        cube[a][b][c] = change;
    ...
    public static class UnionFind {
        ...
        public int[] size;
        public int[] help;
        public int sets;//当前连通块的总数量

        //red：立方体中红色方块的位置（1表示红色，0表示白色）
        ...
        //初始化单个方块，并与相邻白色方块建立连通关系
        public void figure(int x, int y, int z) {
            int i = index(x, y, z);//数组压缩
            father[i] = i;
            size[i] = 1;
            sets++;
            union(i, x - 1, y, z);
