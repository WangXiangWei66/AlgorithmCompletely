...
public class Code01_AStarAlgorithm {
    ...
    //dis：当前节点到起点的已走距离
    public static void add1(int pre, int row, int col, int n, int m, int[][] map, boolean[][] closed, PriorityQueue<int[]> heap) {
    ...
    public static int minDistance2(int[][] map, int startX, int startY, int targetX, int targetY) {
        ...
        int n = map.length;
        int m = map[0].length;
        // 队列元素是int数组：[已走距离(fromDistance), 预估距离(h), 行坐标(row), 列坐标(col)]
    ...
    public static void add2(int pre, int row, int col, int targetX, int targetY, int n, int m, int[][] map, boolean[][] closed, PriorityQueue<int[]> heap) {
        if (row >= 0 && row < n && col >= 0 && col < m && map[row][col] != 0 && !closed[row][col]) {
