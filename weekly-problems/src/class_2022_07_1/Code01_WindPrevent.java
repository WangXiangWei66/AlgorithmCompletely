...
public class Code01_WindPrevent {
    ...
    public static int bestHeight1(int[][] matrix, int k) {
        ...
        for (int startRow = 0; startRow < n; startRow++) {
            int bottleNack = Integer.MAX_VALUE;//当前防风带的整体防风高度
            for (int col = 0; col < m; col++) {
                ...
                bottleNack = Math.min(bottleNack, height);
            }
            ans = Math.max(ans, bottleNack);
        }
        return ans;
    }
    ...
    public static int bestHeight2(int[][] matrix, int k) {
        int n = matrix.length;
        int m = matrix[0].length;
        //第col列的队列范围由 windowLR[col][0]（左边界）和 windowLR[col][1]（右边界）界定
        int[][] windowMaxs = new int[m][n];//每列的单调队列
        int[][] windowLR = new int[m][2];//该列队列的左右边界
        //保证了队列中是前k行的最大值索引
        for (int i = 0; i < k; i++) {
            addRow(matrix, m, i, windowMaxs, windowLR);
        }
        int ans = bottleNecks(matrix, m, windowMaxs, windowLR);//当前窗口的整体防风高度
        for (int i = k; i < n; i++) {
            addRow(matrix, m, i, windowMaxs, windowLR);
            deleteRow(m, i - k, windowMaxs, windowLR);
            ans = Math.max(ans, bottleNecks(matrix, m, windowMaxs, windowLR));
        }
        return ans;
    }
    ...
    public static void addRow(int[][] matrix, int m, int row, int[][] windowMaxs, int[][] windowLR) {
        for (int col = 0; col < m; col++) {
            while (windowLR[col][0] != windowLR[col][1] && matrix[windowMaxs[col][windowLR[col][1] - 1]][col] <= matrix[row][col]) {
                windowLR[col][1]--;
            }
            windowMaxs[col][windowLR[col][1]++] = row;//加入新的列，更高
        }
    }
    ...
    public static int bottleNecks(int[][] matrix, int m, int[][] windowMaxs, int[][] windowLR) {
