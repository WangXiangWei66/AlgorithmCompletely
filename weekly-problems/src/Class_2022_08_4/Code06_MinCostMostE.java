...
public class Code06_MinCostMostE {
    public static class Info {
        public int maxGood;//最多的好E
        public int minCost;//最小修改次数
    ...
    // 最多的好e, 最小的代价
    public static Info wang(char[] s, int i, char prepre, char pre) {
        if (i == s.length) {
            return new Info(0, 0);
        }
        ...
        Info info1 = wang(s, i + 1, pre, 'r');//递归处理后续字符，累加好"e"数量和修改次数
        int p1Value = cur1Value + info1.maxGood;
        int p1Cost = cur1Cost + info1.minCost;

        // 可能性2 i-1 [i] -> e
        int cur2Value = 0;
        int cur2Cost = s[i] == 'e' ? 0 : 1;
        Info info2 = wang(s, i + 1, pre, 'e');
        int p2Value = cur2Value + info2.maxGood;
        int p2Cost = cur2Cost + info2.minCost;

        // 可能性3 i-1 [i] -> d
        int cur3Value = prepre == 'r' && pre == 'e' ? 1 : 0;
        int cur3Cost = s[i] == 'd' ? 0 : 1;
        Info info3 = wang(s, i + 1, pre, 'd');
        int p3Value = cur3Value + info3.maxGood;
        int p3Cost = cur3Cost + info3.minCost;

        int mostE = 0;
        int minCost = Integer.MAX_VALUE;
        // 比较三种可能性，选择"好e"最多且修改次数最少的方
        if (mostE < p1Value) {
            mostE = p1Value;
            minCost = p1Cost;
