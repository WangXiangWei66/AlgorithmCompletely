...
public class Code05_TravelMinFuel {
    ...
    public static int minFuel(int[] a, int[] b, int n) {
        ...
        // a 号，dfn[a] == 0 没遍历过！
        // dfn[a] != 0 遍历过！
        int[] dfn = new int[n + 1]; //标记节点是否被访问
        int[] size = new int[n + 1];//每个节点为根的子树的节点数量
        // 所有居民要汇总吗？
        // a为头的树，所有的居民是要向a来汇聚
        // cost[a] : 所有的居民要向a来汇聚，总油量的耗费
        int[] cost = new int[n + 1];//每个居民汇聚到某个节点的总油耗
        cnt = 0;
    ...
    public static void dfs(ArrayList<ArrayList<Integer>> graph, int cur, int[] dfn, int[] size, int[] cost) {
        ...
        for (int next : graph.get(cur)) {
            if (dfn[next] == 0) {
                ...
                size[cur] += size[next];
                cost[cur] += cost[next];
                cost[cur] += (size[next] + 4) / 5;//计算子树到父节点的油耗
            }
        }
    }
    ...
    public static long minimumFuelCost(int[][] roads, int seats) {
        ...
        int[] size = new int[n + 1];//表示以cur为根的子树中，左右节点的数量
    ...
    public static void dfs(int cur, int father, int seats, ArrayList<ArrayList<Integer>> graph, int[] size, long[] cost) {
        size[cur] = 1;//初始化节点为1，包含当前节点的居民
        for (int next : graph.get(cur)) {
            if (next != father) {
