...
public class Code03_FindPosition {
    public static int[] find(int n, int[] a, int[] b, int[] c, int ad, int bd, int cd) {
        ...
        //下面开始进行BFS
        int[] cur = {x1[0] - r1, x1[1]}; //初始化搜索起始点
        Queue<int[]> queue = new LinkedList<>();//记录BFS节点存储
        HashSet<String> visited = new HashSet<>();//记录已访问过的目标节点
        ArrayList<int[]> ans = new ArrayList<>();//记录所有候选点
        queue.add(cur);
        visited.add(cur[0] + "_" + cur[1]);//将初始点标记为已访问
